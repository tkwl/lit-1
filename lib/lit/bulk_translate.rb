...
module Lit
  class BulkTranslate
    def self.call(locale_keys:, source_locale:)
      ...
      if source_locale.present?
        source_locale_ids = Lit::Locale.where(locale: source_locale).pluck(:id)
        source_localizations_scope = localizations_scope.where(locale_id: source_locale_ids) unless source_locale_ids.empty?
      end
...
# TODO Finish bulk translator from this point down
      case format
      when :yaml
